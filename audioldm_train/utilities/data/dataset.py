...
class EEGDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        embedding = self.load_eeg_embedding(index)
        audio_embedding = self.load_audio_embedding(index)

        data = {
            "embedding": embedding.detach(),  # Load EEG embedding instead of text/captions
    ...
    def load_embedding(self, file_path):
        ...
        try:
            ...
            embedding_cuda = embedding.clone().to('cuda')
            return embedding_cuda
        except Exception as e:
            print(f"Error loading embedding: {file_path}, {e}")
            sys.exit(1)


    def load_eeg_embedding(self, index):
        """Load the EEG embedding file based on the path provided in the dataset."""
        datum = self.data[index]
        embedding_path = datum["embedding"]  # Path to the EEG embedding file

        if os.path.exists(embedding_path):
            embedding = self.load_embedding(embedding_path)  # Use the provided function
            return embedding
        else:
            raise FileNotFoundError(f"Embedding file {embedding_path} not found, {self.count}")
    
    def load_audio_embedding(self, index):
        """Load the EEG embedding file based on the path provided in the dataset."""
        datum = self.data[index]
        embedding_path = datum["audio_embedding"]  # Path to the EEG embedding file
